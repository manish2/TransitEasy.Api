...
namespace TransityEasy.Api.Core.Extensions
{
    ...
    {
        public static async Task<T> GetFromJsonAsync<T>(this HttpClient client, string requestUri, HashSet<HttpStatusCode> codesToOverride)
        {
        ...
        public static async Task<(string, HttpStatusCode)> GetPayloadWithHttpCodeAsync(this HttpClient client, string requestUri, HashSet<HttpStatusCode> codesToOverride)
        {
            var response = await client.GetAsync(requestUri);
